...
namespace LearnLanguages.Business
{
  ...
  public class DeleteMultiLineTextsReadOnly : Common.CslaBases.ReadOnlyBase<DeleteMultiLineTextsReadOnly>
  {
    ...
    public static async Task<DeleteMultiLineTextsReadOnly> CreateNewAsync(string username)
    {//left off here. need to create criteria. will use a for loop on collection criteria that calls mltdal.delete(id) foreach id.
      var result = await DataPortal.CreateAsync<DeleteMultiLineTextsReadOnly>(username);
      return result;
    }
