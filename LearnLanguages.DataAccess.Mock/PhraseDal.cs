  ...
  public class PhraseDal : IPhraseDal
  {
    public Result<PhraseDto> New(object criteria)
    {
      ...
      try
      {
        ...
        { 
          Id = Guid.NewGuid(),
          LanguageId = SeedData.DefaultLanguageId
        };
    ...
    public Result<PhraseDto> Fetch(Guid id)
    {
      ...
      try
      {
        var results = from item in SeedData.Phrases
                      where item.Id == id
                      select item;
    ...
    public Result<PhraseDto> Update(PhraseDto dto)
    {
      ...
      try
      {
        ...
        if (results.Count() == 1)
        {
          ...
          SeedData.Phrases.Remove(PhraseToUpdate);
          dto.Id = Guid.NewGuid();
          SeedData.Phrases.Add(dto);
          retResult = Result<PhraseDto>.Success(dto);
        }
        else
        {
    ...
    public Result<PhraseDto> Insert(PhraseDto dto)
    {
      ...
      try
      {
        ...
        if (results.Count() == 0)
        {
          ...
          if (dto.LanguageId == Guid.Empty || !SeedData.ContainsLanguageId(dto.LanguageId))
    ...
    public Result<PhraseDto> Delete(Guid id)
    {
      ...
      try
      {
        ...
        if (results.Count() == 1)
        {
          ...
          SeedData.Phrases.Remove(PhraseToRemove);
    ...
    public LearnLanguages.Result<ICollection<PhraseDto>> GetAll()
    {
      ...
      try
      {
        var allDtos = new List<PhraseDto>(SeedData.Phrases);
